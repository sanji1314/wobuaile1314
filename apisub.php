...
switch($act){
    ...
    $stmt = $DB->prepare("UPDATE qingka_wangke_user SET yqprice = :yqprice WHERE uid = :uid");  
    ...
    if (!is_array($raw_data) || !isset($raw_data['id'], $raw_data['sort'], $raw_data['name'])) {  
    ...
    $stmt = $DB->prepare("UPDATE `qingka_wangke_class` SET `fenlei` = :name WHERE `name` LIKE :sort_pattern");  
    ...
    $stmt = $DB->prepare("SELECT * FROM qingka_wangke_user WHERE uid = :uid LIMIT 1");  
...
case 'yjdj':
    if ($userrow['uid'] == 1) {
        ...
        $DB->query("insert into qingka_wangke_fenlei (sort,name,status,time) values ('0','{$a["name"]}','1',NOW())");
        $b = $DB->get_row("select * from qingka_wangke_fenlei where name='{$a["name"]}' ORDER BY id DESC LIMIT 0,1");
        $data = $result1["data"];
        $numItems = count($data);
        $i = 0;
        foreach ($data as $k => $value) {
            if ($value['fenlei'] == $category) { // 对比用户输入的分类ID
                ...
                $DB->query("insert into qingka_wangke_class (name, getnoun, noun, fenlei, queryplat, docking, price, sort, content) values ('{$value['name']}', '{$value['cid']}', '{$value['cid']}', '{$b["id"]}', '$hid', '$hid', '{$price}', '{$sort}', '{$value['content']}')");
                $i++;
            }
        }
        jsonReturn(1, "已上架{$a["name"]}的全部分类的项目，共计{$i}个，并自动新建分类到【{$b["name"]}】中，价格、排序和内容已更新");
    }
    break;
    case 'yjsj':
	    $b=$DB->query("select * from qingka_wangke_class where 1=1 ");
	    $count1=$DB->count("select count(*) from qingka_wangke_class where 1=1");
...
// 		}	
	...
	case 'user_kh':
    	...
    	$DB->query("update qingka_wangke_user set kuahu='$a' where uid='{$uid}' ");
    	wlog($userrow['uid'],$b,"{$b}[UID {$uid}]成功",0);
    	jsonReturn(1,"操作成功");
    ...
    break;
    case 'tuboshu_dialogues':
		include('./Checkorder/tuboshu.php');

		$instance = new TuBoShuPost();
		exit(json_encode($instance->getDialogues()));
		break;
	case 'tuboshu_templates':
		include('./Checkorder/tuboshu.php');

		$instance = new TuBoShuPost();
		exit(json_encode($instance->getDialogueTemplates()));
		break;
		// 下单
	case 'tuboshu_submit':
		include('./Checkorder/tuboshu.php');
		...
		$instance = new TuBoShuPost();
		$dialogue_id = intval(trim(strip_tags(daddslashes($_POST['id']))));
		$options = $_POST['options'];


		$clientip = real_ip();

		$dialogues = $instance->getDialogues();

		// 查找 id 为 $dialogue_id 的对话
		$dialogue = null;
		foreach ($dialogues as $k => $item) {
		...
		if (!$dialogue) {
			exit('{"code":-1,"msg":"对话不存在"}');
		}

		$price = $dialogue['point'];
		if ($dialogue['name'] == "论文撰写") {
			$sectionPrice = $instance->getPaperPrice()['first'];
			$tigang = $options['tigang'];
			if (!$tigang) {
				exit('{"code":-1,"msg":"题纲不能为空，请先生成或输入题纲再生成"}');
			}

			// sectionLength = max(outline.lines().map { it.trim() }.filter { it.startsWith("## ") }.size, 10)
			$sectionLength = max(count(array_filter(explode("\n", $tigang), function ($line) {
				return trim($line) && strpos(trim($line), '## ') === 0;
			})), 10);
			$price = $sectionPrice * $sectionLength;
		}
		...
		$post_result = $instance->postDialogue($dialogue_id, $options);

		if (!$post_result['success']) {
			exit('{"code":-1,"msg":"提交失败, ' . $post_result['message'] . '"}');
		}

		$title = $options['prompt'];
		$source_id = $post_result['data']['id'];
		$sql = "insert into qingka_wangke_dialogue (uid,title,state,addtime,ip,source_id,dialogue_id,point,download_url) values ('{$userrow['uid']}', '$title', 'PENDING','$date','$clientip', '$source_id','$dialogue_id','$price', '') ";
		$is = $DB->query($sql); //将对应课程写入数据库
		
		if ($post_result['data']['point']) {
			$price = max($price, $post_result['data']['point']);
		}
	...
	case 'tuboshu_orders':
		// 查询历史订单
		$orders = $DB->query("SELECT * FROM qingka_wangke_dialogue WHERE uid='{$userrow['uid']}' ORDER BY id DESC");
		$data = array();
		while ($row = $DB->fetch($orders)) {
			$data[] = $row;
		}
	...
	case 'tuboshu_task':
		...
		$order = $DB->get_row("select * from qingka_wangke_dialogue where id='$id' limit 1");
		if (!$order) {
			exit('{"code":-1,"msg":"订单不存在"}');
		}
		if ($order['uid'] != $userrow['uid']) {
			exit('{"code":-1,"msg":"无权限"}');
		}

		exit(json_encode(array('data' => $instance->getOrderResult($order['source_id']))));
		break;
	case 'tuboshu_update':
		include('./Checkorder/tuboshu.php');
		...
		$source_order = $instance->getOrder($order['source_id']);
		$state = $source_order['status'];
		if ($state != $order['state']) {
			$DB->query("update qingka_wangke_dialogue set `state` = '{$state}' where `id` = '{$order['id']}'");
		}
		if ($source_order['downloadUrl']) {
			$downloadUrl = $source_order['downloadUrl'];
			$DB->query("update qingka_wangke_dialogue set `download_url` = '{$downloadUrl}' where `id` = '{$order['id']}'");
		}

		exit('{"code":1,"msg":"更新成功"}');
		break;
	case 'tuboshu_paper_price':
		include('./Checkorder/tuboshu.php');

		$instance = new TuBoShuPost();
		exit(json_encode(array('data' => $instance->getPaperPrice())));
		break;
	case 'tuboshu_generate_outline':
		include('./Checkorder/tuboshu.php');

		$instance = new TuBoShuPost();

		$price = $instance->getPaperPrice()['second'];
		if ($userrow['money'] < $price) {
			exit('{"code":-1,"msg":"余额不足"}');
		}
		...
		// 所有 post 请求体
		$param = $_POST['data'];
		$outline = $instance->generateOutline($param);

		if ($outline) {
			$DB->query("update qingka_wangke_user set money=money-'{$price}' where uid='{$userrow['uid']}' limit 1 ");
			wlog($userrow['uid'],"添加任务","  添加论文任务扣除{$price}元！",-$price);
		}
		
		exit(json_encode(array('code' => 1, 'data' => $outline)));
		break;
		
	case 'get_orders':
		// 获取所有非已完成订单的ID
		$orders = array();
		$query = $DB->query("SELECT oid FROM qingka_wangke_order WHERE status!='已完成' AND status!='已取消' ORDER BY oid ASC");
	...
	case 'get_user_orders':
		...
		$uid = intval($_GET['uid']);
		if($uid <= 0){
			exit(json_encode(array('code'=>-1, 'msg'=>'无效的用户ID')));
		}
		
		$orders = array();
		$query = $DB->query("SELECT oid FROM qingka_wangke_order WHERE uid='{$uid}' AND status!='已完成' AND status!='已取消' ORDER BY oid ASC");
		while($row = $DB->fetch($query)){
			$orders[] = $row['oid'];
		}
	...
	case 'get_order_info':
		...
		$oid = intval($_GET['oid']);
		if($oid <= 0){
			exit(json_encode(array('code'=>-1, 'msg'=>'无效的订单ID')));
		}
		...
		exit(json_encode($result));
		break;
    case 'wkupdate'://进度刷新，给monitor.php使用
           $oid=intval($_POST['oid']);
           if(!$oid) {
               exit(json_encode(array('code'=>-1, 'msg'=>'订单ID不能为空')));
           }
